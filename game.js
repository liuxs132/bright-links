...
function shareUrl() {
  const canonical = document.querySelector('meta[name="bright-links:canonical"]')?.getAttribute("content")?.trim();
  if (canonical) return canonical;
  if (window.location.protocol === "file:") return "https://bright-links-gold.vercel.app/";
...
function copyResult() {
  ...
  const grid = history.map((color) => (color === "#d34646" ? "⬛" : colorMap.get(color) ?? "🟩")).join("");
  const result = [
    "Bright Links",
    scoreLine,
    revealed ? "Revealed" : `Mistakes: ${mistakes}/${MAX_MISTAKES}`,
